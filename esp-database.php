<?php
    ...
    function getAllOutputStates($board) {
        ...
        if ($result = $conn->query($sql)) {
            // Automatically reset all outputs with state '1' to '0' after getting the data
            $resetSql = "UPDATE Outputs SET state='0' WHERE board='" . $board . "' AND state='1'";
            $conn->query($resetSql);
    ...
    function resetOutputStates($board) {
        global $servername, $username, $password, $dbname;
        ...
        $sql = "UPDATE Outputs SET state='0' WHERE board='" . $board . "' AND state='1'";

       if ($conn->query($sql) === TRUE) {
            return "Output states reset successfully";
        }
        else {
            return "Error: " . $sql . "<br>" . $conn->error;
        }
